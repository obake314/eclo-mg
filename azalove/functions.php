...
if ( ! function_exists( 'azarashilove_posted_on' ) ) :
	function azarashilove_posted_on() {
		...
		echo '<div class="posted-on">' . sprintf( esc_html_x( '%s', 'post date', 'azarashilove' ), $time_string ) . '</div>';
	}
endif;
...
if ( ! function_exists( 'azarashilove_entry_footer' ) ) :
	function azarashilove_entry_footer() {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'azarashilove' ), array( 'span' => array( 'class' => array() ) ) ),
...
function azarashilove_body_classes( $classes ) {
	if ( ! is_singular() ) {
		$classes[] = 'hfeed';
	}
...
add_filter( 'body_class', 'azarashilove_body_classes' );

function azarashilove_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'azarashilove_pingback_header' );
